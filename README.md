...
- `YOUTUBE_API_KEY` (선택) — 곡 등록 폼에서 제목 입력하면 유튜브 검색 결과가
  떠서 클릭으로 링크를 채울 수 있게 해주는 기능에 씁니다. Google Cloud
  Console에서 프로젝트 만들고 **YouTube Data API v3**를 사용 설정한 뒤,
  API 키를 발급하면서 "API 제한사항"을 YouTube Data API v3만으로 제한해서
  씁니다. 무료 할당량 안에서(하루 검색 약 100회) 충분합니다. 이 값이 없어도
  나머지 관리자 기능은 정상 동작하고, 검색창엔 안내 에러만 뜹니다.
